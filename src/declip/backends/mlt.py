...
from declip.compilers.mlt import bitrate_for, compile_project, compile_to_string, compile_xml, encoder_for, resolve_output_path
...
def render(project: Project, project_dir: Path, out: OutputManager, total_duration: float | None = None) -> bool:
    ...
    with tempfile.NamedTemporaryFile(suffix=".mlt", mode="w", delete=False) as handle:
        handle.write(compiled.xml); xml_path = handle.name
    ...
    command = [melt_bin, xml_path, "-consumer", f"avformat:{output_path}", "real_time=-1", f"width={width}", f"height={height}", f"vcodec={encoder_for(normalized)}", f"vb={bitrate_for(normalized)}", f"acodec={normalized.output.audio_codec}", f"ab={normalized.output.audio_bitrate}", "terminate_on_pause=1"]
    ...
    for raw_line in proc.stderr:
        line = raw_line.decode(errors="replace"); stderr_lines.append(line)
        progress = _parse_melt_progress(line, total_frames)
        if progress is not None:
            out.progress(progress)
    proc.wait(); Path(xml_path).unlink(missing_ok=True)

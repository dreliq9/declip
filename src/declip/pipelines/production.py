...
def _run_async(coro: Coroutine[Any, Any, Any]) -> Any:
    """Run an async provider call from sync code, even inside an active event loop."""
    try:
    ...
    def runner() -> None:
        ...
        except BaseException as exc:  # preserve provider exception type
            error.append(exc)
...
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text"""
    lines = [header]
    for group_start in range(0, len(words), 6):
        group = words[group_start:group_start + 6]
        if not group:
            continue
        if style in {"karaoke", "bold"}:
            text = " ".join(
                f"{{\\kf{max(1, int((word['end'] - word['start']) * 100))}}}{word['word']}"
                for word in group
            )
        else:
...
def auto_caption(
    ...
    ass_path.write_text(generate_ass(words, style=style, resolution=resolution), encoding="utf-8")
    if ass_only:
        ...
        return PipelineResult(
            ...
            details={"words": len(words), "style": style, "language": transcription.language},
        )
    ...
    ok, error = _run_ffmpeg(
        ["ffmpeg", "-y", "-i", input_file, "-vf", f"ass='{escaped}'", "-c:a", "copy", output_path],
        timeout=600,
    )
    if not ok:
    ...
    message = (
        ...
        f"Words: {len(words)}, Language: {transcription.language}\n"
    ...
    return PipelineResult(
        ...
        details={"ass_path": str(ass_path), "words": len(words), "style": style, "language": transcription.language},
    )
...
def tts(
    ...
    details: dict[str, Any] = {"voice": voice}
...
def tts_voices(language: str = "en") -> PipelineResult:
    ...
    lines = [f"Voices for '{language}' ({len(filtered)}):"] + [
        f"  {voice['ShortName']} — {voice.get('Gender', '')}, {voice['Locale']}" for voice in filtered
    ]
...
def _measure_loudness(input_file: str, lufs: int) -> str:
    ...
    try:
        measure = subprocess.run(
            ["ffmpeg", "-i", input_file, "-af", f"{fallback}:print_format=json", "-f", "null", "-"],
            capture_output=True,
            text=True,
            timeout=300,
        )
    ...
    try:
        ...
        return (
            ...
            f"offset={data.get('target_offset','0')}:linear=true"
        )
    except (json.JSONDecodeError, KeyError):
        return fallback


def _reframe_filter(width: int, height: int, aspect: str, strategy: str) -> tuple[str, bool]:
    if strategy == "blur_bg" and aspect != "16:9":
        return (
            f"[0:v]split[bg][fg];"
            f"[bg]scale={width}:{height}:force_original_aspect_ratio=increase,crop={width}:{height},boxblur=20:5[blurred];"
            f"[fg]scale={width}:{height}:force_original_aspect_ratio=decrease[sharp];"
            f"[blurred][sharp]overlay=(W-w)/2:(H-h)/2[vout]",
            True,
        )
    if strategy == "center_crop" and aspect != "16:9":
        return (
...
def platform_export(
    ...
    from declip.probe import probe
    try:
        has_audio = bool(probe(input_file).audio_codec)
    except Exception:
        has_audio = True

    output_dir = output_dir or str(Path(input_file).stem + "_exports")
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    loudness_filters = (
        {
            lufs: _measure_loudness(input_file, lufs)
            for lufs in {PLATFORM_SPECS[platform]["lufs"] for platform in selected}
        }
        if has_audio
        else {}
    )
    ...
    for platform in selected:
        ...
        video_filter, complex_filter = _reframe_filter(width, height, spec["aspect"], reframe)
        output = str(Path(output_dir) / f"{platform}.mp4")
        command = ["ffmpeg", "-y", "-i", input_file]
        if complex_filter:
            command += ["-filter_complex", video_filter, "-map", "[vout]"]
            if has_audio:
                command += ["-map", "0:a?"]
        else:
            command += ["-vf", video_filter]
        if has_audio:
            command += ["-af", loudness_filters[spec["lufs"]]]
        command += [
        ...
        if ok:
            successes.append(f"  {spec['label']}: {_file_info(output)}")
    ...
    return PipelineResult(
        success=bool(successes) and not failures,
        message="\n".join(lines),
        output_path=output_dir,
        details={"outputs": outputs, "failures": failures},
...
def _video_has_audio(path: str) -> bool:
    from declip.probe import probe
    try:
        return bool(probe(path).audio_codec)
    except Exception:
        return True


def _mix_narration(video_file: str, narration_file: str, start: float, output_path: str) -> tuple[bool, str]:
    delay = max(0, int(start * 1000))
    if _video_has_audio(video_file):
        graph = (
            f"[1:a]adelay={delay}|{delay}[narration];"
            f"[0:a][narration]amix=inputs=2:duration=first:normalize=0[aout]"
        )
        command = [
        ...
        graph = f"[1:a]adelay={delay}|{delay}[aout]"
        command = [
            "ffmpeg", "-y", "-i", video_file, "-i", narration_file,
            "-filter_complex", graph, "-map", "0:v", "-map", "[aout]",
...
def _mix_music(video_file: str, music_file: str, volume: float, output_path: str) -> tuple[bool, str]:
    if _video_has_audio(video_file):
        graph = (
            ...
            f"[music][0:a]sidechaincompress=threshold=0.02:ratio=8:attack=200:release=1000:level_sc=1[ducked];"
            f"[ducked][0:a]amix=inputs=2:duration=first:normalize=0[aout]"
        )
        command = [
        ...
        command = [
            ...
            "-af", f"volume={volume}", "-c:a", "aac", "-b:a", "192k", "-shortest", output_path,
        ]
    return _run_ffmpeg(command, timeout=600)
...
def storyboard(
    ...
    if transition_duration <= 0 or music_volume < 0:
        return PipelineResult(
            success=False,
            message="Error: transition_duration must be positive and music_volume non-negative",
            error="invalid parameters",
        )

    from declip.schema import Project, TransitionType
    normalized_default_transition = _schema_transition(transition)
    try:
        TransitionType(normalized_default_transition)
    except ValueError:
        return PipelineResult(success=False, message=f"Error: unknown transition '{transition}'", error="invalid transition")
    ...
    for index, shot in enumerate(shot_list):
        ...
        if "transition" in shot:
            normalized = _schema_transition(str(shot["transition"]))
            try:
                TransitionType(normalized)
            except ValueError:
                return PipelineResult(success=False, message=f"Error: unknown transition '{shot['transition']}' in shot {index}", error="invalid transition")
    ...
    with tempfile.TemporaryDirectory(prefix="declip_storyboard_") as directory:
        ...
        for index, shot in enumerate(shot_list):
            if shot.get("narration") and voice:
                ...
                if narration.success:
                    ...
                    try:
                        narration_duration = probe(narration_path).duration
                    except Exception:
                        narration_duration = narration.details.get("duration_seconds", 0)
            ...
            if "duration" not in shot:
                try:
                    shot["duration"] = probe(shot["asset"]).duration
                except Exception:
                    shot["duration"] = 5.0
                    warnings.append(f"Duration probe failed for shot {index}; using 5.0s")

        clips: list[dict] = []
        for index, shot in enumerate(shot_list):
            clip: dict[str, Any] = {
                "asset": str(Path(shot["asset"]).resolve()),
                "start": 0 if index == 0 else "auto",
                "duration": float(shot.get("duration", 5.0)),
                "filters": [],
            }
            if index > 0:
                clip["transition_in"] = {
                    "type": _schema_transition(str(shot.get("transition", transition))),
        ...
        visual_output = (
            str(work / "visuals.mp4")
            if voice or music or caption_style
            else str(Path(output_path).resolve())
        )
        project = Project.model_validate(
            {
                "version": "1.0",
                "timeline": {"tracks": [{"id": "main", "clips": clips}], "audio": []},
                "output": {"path": visual_output},
            }
        )
        render_out = OutputManager(json_mode=False, quiet=True)
        if not ffmpeg_backend.render(project, Path("."), render_out, total_duration=None):
            return PipelineResult(
                success=False,
                message="Error: storyboard visual render failed",
                error=render_out.get_log(),
            )

        resolved_clips = build_render_plan(project, Path(".")).project.timeline.tracks[0].clips
        ...
        for index, shot in enumerate(shot_list):
            ...
            ok, error = _mix_narration(
                current,
                narration,
                float(resolved_clips[index].start),
                mixed,
            )
            if not ok or not Path(mixed).exists():
                warnings.append(f"Narration mix failed for shot {index}: {error}")
            else:
                current = mixed
        ...
        if music:
            if not Path(music).exists():
                ...
                if not ok:
                    warnings.append(f"Music mix failed: {error}")
                    shutil.copy2(current, final_pre_caption)
        ...
        duration = max(
            float(clip.start) + float(clip.duration or 0)
            for clip in resolved_clips
        )
        lines = [f"Storyboard assembled: {len(shot_list)} shots", f"Duration: {duration:.1f}s"]
        if voice:
            voiced = sum(1 for shot in shot_list if shot.get("_narration_audio"))
            lines.append(f"Narration: {voiced} shots voiced ({voice})")
        if music:
        ...
        return PipelineResult(
            success=True,
            message="\n".join(lines),
            output_path=final_pre_caption,
            details={"shots": len(shot_list), "duration_seconds": duration, "warnings": warnings},
        )

...
No FFmpeg subprocess is executed here; execution belongs to ``declip.backends``.
"""
...
from declip.filters.ffmpeg import (
    build_audio_filters,
    build_video_filters,
...
def can_handle(project: Project) -> bool:
    if len(project.timeline.tracks) != 1 or project.timeline.audio:
        return False
    return not any(clip.position for clip in project.timeline.tracks[0].clips)
...
def _clip_input_args(clip: Clip, asset: str) -> list[str]:
    args: list[str] = []
    if clip.freeze_frame is not None:
        ...
        duration = _source_duration(clip)
    if duration > 0:
        args += ["-t", str(duration)]
    return args + ["-i", asset]
...
def _has_audio(asset_path: str) -> bool:
    try:
        ...
        if not ffprobe:
            return True
        try:
            proc = subprocess.run([ffprobe, "-v", "error", "-select_streams", "a", "-show_entries", "stream=index", "-of", "csv=p=0", asset_path], capture_output=True, text=True, timeout=10)
    ...
    for index in range(1, len(labels)):
        ...
        if clip.transition_in is not None:
            ...
            if duration >= output_duration or duration >= current_duration:
                raise CompilationError(f"transition into clip {index} ({duration}s) must be shorter than both adjacent compiled durations ({output_duration}s, {current_duration}s)")
...
def _timeline_command(project: Project, project_dir: Path) -> list[str]:
    if not can_handle(project):
        raise CompilationError("FFmpeg compiler supports one unpositioned video track and no dedicated audio tracks; use the MLT backend for this project")
    clips = sorted(project.timeline.tracks[0].clips, key=lambda clip: float(clip.start))
    if not clips:
        raise CompilationError("project contains no clips")
    command = ["ffmpeg", "-y"]
    primary_indices, asset_paths = [], []
    input_count = 0
    for clip in clips:
        asset = resolve_asset(clip.asset, project_dir)
        asset_paths.append(asset)
        primary_indices.append(input_count)
        command += _clip_input_args(clip, asset)
        input_count += 1
    ...
    parts, video_labels, audio_labels = [], [], []
